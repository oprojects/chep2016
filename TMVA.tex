...
\usepackage{graphicx}
\begin{document}
\title{New Machine Learning Developments in ROOT}

\author{S. V. Gleyzer$^1$, L. Moneta$^2$, Omar A. Zapata$^3$, add your name here $^4$}

\address{$^1$ University of Florida}
\address{$^2$ CERN}
\address{$^3$ University of Antioquia and Metropolitan Institute of Technology}
\address{$^4$ add you university}
\ead{Sergei.Gleyzer@cern.ch, Lorenzo.Moneta@cern.ch, Omar.Zapata@cern.ch, add your email here}

\begin{abstract}
ROOT provides advanced statistical methods needed by the LHC experiments to analyze their data \cite{Antcheva20092499}. These include machine learning tools for classification, regression and clustering. TMVA, a toolkit for multi-variate analysis in ROOT  \cite{Hocker:2007ht}, provides these machine learning methods.
\end{abstract}
...
\section{TMVA Interfaces}
Interfaces to External ML Tools 
\begin{itemize}  
\item RMVA interface to R
\item PyMVA interface to scikit-learn
\item KMVA interface to Keras
\subitem High-level interface to Theano,
\subitem TensorFlow deep-learning libraries 
\end{itemize}

\subsection{RMVA interface to R}
TODO by omar
\subsection{PyMVA interface to scikit-learn}
TODO by omar
\subsection{KMVA interface to Keras}
TODO by Stefan
...
\section{New regression features}
TODO by Andrew
